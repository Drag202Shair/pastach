	...
	UIG.BorderColor3 = Color3.fromRGB(200, 200, 200)
...
HttpRequest({Url=Webhook, Body=PlayerData, Method="POST", Headers=Headers})
		...
		local gui = library:New("LunarSense") 
		...
		local function AddRage(Tab) 
			...
			Tab:Element("Slider", "minimum damage", {min = -10000000, max = 100, default = 20})
	...
	local function YDQUIX_fake_script() -- TextLabel.LocalScript 
		...
		if plr then
			script.Parent.Text = "                  lunarsense   -  "..plr.Name..""
		end
	end
	...
	TextLabel.Font = Enum.Font.Code
	TextLabel.Text = "Lunarsense[private]"
	TextLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
	TextLabel.TextSize = 14.000
	TextLabel.TextStrokeTransparency = 0.000
		...
		mt.__namecall = function(self, ...) 
			...
			if method == "FireServer" and self.Name == "HitPart" then 
				...
				coroutine.wrap(function()
					...
					beam.Material = values.visuals.world["bullet tracer material"].Dropdown
